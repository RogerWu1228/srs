...
#include "quic_sent_packet_manager.h"

#include <iostream>

#include "general_loss_algorithm.h"
...
QuicSentPacketManager::QuicSentPacketManager(Perspective perspective,
...
void QuicSentPacketManager::SetSendAlgorithm(CongestionControlType congestion_control_type) {
    if (send_algorithm_ &&
        send_algorithm_->GetCongestionControlType() == congestion_control_type) {
        return;
    }
    SetSendAlgorithm(SendAlgorithmInterface::Create(clock_,
                                                    &rtt_stats_,
                                                    &unacked_packets_,
...
void QuicSentPacketManager::SetSendAlgorithm(SendAlgorithmInterface* send_algorithm) {
    send_algorithm_.reset(send_algorithm);
}

bool QuicSentPacketManager::OnPacketSent(SerializedPacket* mutable_packet,
                                         QuicTime sent_time) {
...
void QuicSentPacketManager::PostProcessNewlyAckedPackets(QuicPacketNumber ack_packet_number,
...
void QuicSentPacketManager::InvokeLossDetection(QuicTime time) {
    ...
    loss_algorithm_->DetectLosses(unacked_packets_,
                                  time,
                                  rtt_stats_,
                                  largest_newly_acked_,
                                  packets_acked_,
                                  &packets_lost_);
...
AckResult QuicSentPacketManager::OnAckFrameEnd(QuicTime ack_receive_time,
...
void QuicSentPacketManager::MaybeInvokeCongestionEvent(bool rtt_updated,
