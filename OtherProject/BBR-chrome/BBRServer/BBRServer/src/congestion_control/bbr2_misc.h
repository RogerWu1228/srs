...
#include "chrome_windowed_filter.h"
...
template <typename T>
 inline Limits<T> MinMax(T min, T max) {
  return Limits<T>(min, max);
}

template <typename T>
 inline Limits<T> NoLessThan(T min) {
  return Limits<T>(min, std::numeric_limits<T>::max());
}

template <typename T>
 inline Limits<T> NoGreaterThan(T max) {
  return Limits<T>(std::numeric_limits<T>::min(), max);
}

template <typename T>
 inline Limits<T> Unlimited() {
...
struct  Bbr2Params {
  ...
  int64_t startup_full_loss_count = 20/*
  ...
  QuicTime::Delta probe_rtt_period = QuicTime::Delta::FromMilliseconds(
      200/*GetQuicFlag(FLAGS_quic_bbr2_default_probe_rtt_period_ms)*/);
  ...
  QuicRoundTripCount initial_max_ack_height_filter_window = 4
      /*GetQuicFlag(FLAGS_quic_bbr2_default_initial_ack_height_filter_window)*/;

  // Fraction of unutilized headroom to try to leave in path upon high loss.
  float inflight_hi_headroom = 0.3
  ...
  float loss_threshold = 30/*GetQuicFlag(FLAGS_quic_bbr2_default_loss_threshold)*/;
...
class  Bbr2NetworkModel {
 public:
  ...
  // ///////// added by carr
  // BandwidthSamplerInterface::CongestionEventSample recent_sample;
...
class  Bbr2ModeBase {
 ...
 inline QuicByteCount BytesInFlight(
