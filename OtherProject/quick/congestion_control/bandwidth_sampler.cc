...
void BandwidthSampler::OnPacketSent(
  ...
  last_sent_packet_ = packet_number;
	  ...
	  dd::OutputDebugInfo("OnPacketSent sent time  delta = %llu\r\n", sent_time.ToDebuggingValue() - _prior_sent_time);
#endif
  _prior_sent_time = sent_time.ToDebuggingValue();
  ...
  //  }
  //}
  _connection_states[packet_number.ToUint64()].reset(new ConnectionStateOnSentPacket(sent_time, bytes,
...
//  if (ack_time.ToDebuggingValue() - _print_acked_rate >= 1000000) {
//#ifdef OUTPUT_VERBOSE_INFO
//		  dd::OutputDebugInfo("bandwidth_sampler acked rate = %f Mbyte/s\r\n", (float)event_sample.sample_max_bandwidth.ToBytesPerSecond() / 1024.0 / 1024.0);
//#endif
//	  _print_acked_rate = ack_time.ToDebuggingValue();
//  }
...
BandwidthSample BandwidthSampler::OnPacketAcknowledged(
    QuicTime ack_time,
    QuicPacketNumber packet_number) {
	ConnectionStateOnSentPacket* sent_packet_pointer = _connection_states[packet_number.ToUint64()].get();
